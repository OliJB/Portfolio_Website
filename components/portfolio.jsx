...
const portfolio = async () => {
  ...
  return (
   <>
   <div className="w-full px-4 sm:px-12 md:px-24 lg:px-56 flex flex-col items-center h-fit mb-16">
   	<h2 className="font-barlow text-4xl text-black mb-2">Portfolio</h2>
			<div className="w-24 h-1 bg-black flex flex-auto mb-14"/>
      	...
      	<div className="w-full p-0 m-0 flex flex-col items-center">
         	{projects.map((project) => (
              ...
              <div className="flex flex-col py-10 pr-10 items-left w-2/3">
                <h2 className="font-barlow text-2xl mb-5">{project.name}</h2>
                {/* Use PortableText to render content */}
                <div className="font-montserrat mb-3 text-sm"><PortableText value={project.content} /></div>
                
                <Link href={project.githubLink} target="_blank" className="mb-3 mt-auto bg-slate-600 rounded-md text-white w-fit py-1 px-3 flex items-center"> {/* Added flex and items-center */}
                  ...
                  <span className="font-montserrat">View Code </span>
                </Link>

                <div className="flex flex-wrap mt-auto">
                  {project.languages.map((language) => (
                    <div className="border-2 rounded-md border-slate-600 mr-4 py-1 px-2 font-montserrat " key={language}>{language}</div>
                  ))}
                </div>
              </div>
         		</div>
